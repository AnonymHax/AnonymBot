...
void ServerBuffer::reset()
{
    m_message.messageData.clear();
    m_remainingBytes = 0;
}

Writer ServerBuffer::writePacket(AbstractServerMessage &message)
{
    qDebug()<<"-----------PACKET ENVOYE ( AnonymServer ) ----------";
...
QList<ServerMessageInfos> ServerBuffer::readPacket(const QByteArray &packetData)
{
    QList<ServerMessageInfos> messages;
    ...
    if (m_remainingBytes > 0)
    {
        ...
        if (m_remainingBytes == 0)
        {
            qDebug()<<"-----------PACKET RECONSTITUE ( AnonymServer ) -------";
    ...
    {
        qDebug()<<"-----------PACKET(S) RECU ("<<packetData.size() - messageTotalLength<<")  ( AnonymServer )----------";
        ...
        while (messageTotalLength < packetData.size())
        {
            ...
            m_message.messageType = (ServerMessageEnum)packetID;
